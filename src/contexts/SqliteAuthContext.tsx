...
import { studentService } from '../lib/db-manager';
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const login = async (studentNumber: string): Promise<{ success: boolean; message: string; otp?: string }> => {
    try {
      ...
      const student = await studentService.getByStudentNumber(studentNumber);
      ...
      if (!student && import.meta.env.DEV) {
        ...
        const defaultStudent: Omit<Student, 'id' | 'createdAt' | 'updatedAt'> = {
          studentNumber,
        ...
        await studentService.create(defaultStudent);
      }
  ...
  const verifyOTP = async (otp: string): Promise<{ success: boolean; message: string }> => {
    try {
      ...
      const student = await studentService.getByStudentNumber(otpStudentNumber);
